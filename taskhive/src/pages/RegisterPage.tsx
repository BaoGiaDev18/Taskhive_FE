...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Giả sử backend có endpoint POST /auth/register
      ...
      // Nếu cần lưu name: localStorage.setItem("userName", response.data.userName);

      window.location.href = "/";
    } catch (err: unknown) {
  // Kiểm tra nếu err là AxiosError (thường do api.post)
  if (axios.isAxiosError(err) && err.response) {
    // Ép kiểu để lấy message từ response.data (nếu backend trả { message: string })
  ...
  return (
    <div className="w-full max-w-md px-4">
          ...
          >
            ...
            >
              <path
                d={
                  showPassword
                    ? "M12 6.5C9.33 6.5 6.95 8.02 5.64 10.5C6.95 12.98 9.33 14.5 12 14.5C14.67 14.5 17.05 12.98 18.36 10.5C17.05 8.02 14.67 6.5 12 6.5ZM12 13C10.62 13 9.5 11.88 9.5 10.5C9.5 9.12 10.62 8 12 8C13.38 8 14.5 9.12 14.5 10.5C14.5 11.88 13.38 13 12 13Z"
      ...
      <div className="mt-6 text-center text-sm">
        ...
        <Link to="/login" className="text-[#F57C00] font-medium hover:underline">
          Sign in
        </Link>
      </div>
    </div>
